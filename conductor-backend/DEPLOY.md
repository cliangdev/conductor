...
gcloud run deploy conductor-backend \
  ...
  --set-secrets="DATABASE_URL=DATABASE_URL:latest,JWT_SECRET=JWT_SECRET:latest,FIREBASE_SERVICE_ACCOUNT_KEY=FIREBASE_SERVICE_ACCOUNT_KEY:latest,RESEND_API_KEY=RESEND_API_KEY:latest,GCP_SERVICE_ACCOUNT_KEY=GCP_SERVICE_ACCOUNT_KEY:latest,GCP_STORAGE_BUCKET_NAME=GCP_STORAGE_BUCKET_NAME:latest,FRONTEND_URL=FRONTEND_URL:latest"
```
