...
/**
 * Translates between {@link NotificationEvent} (the old, notification-shaped envelope) and
...
public class NotificationSignalMapper {
    ...
    public Signal toSignal(NotificationEvent event) {
     ...
     */
    public NotificationEvent toNotificationEvent(Signal signal) {
        return NotificationEvent.of(toEventType(signal.type()), signal.projectId(), signal.flatAttributes(),
                signal.occurredAt());
    }
