...
/**
 * Routes the internal {@link EventType#WORK_ITEM_STATUS_CHANGED} event into the generalized lifecycle
 ...
 *
 * <p>Plugged into {@code NotificationDispatcher.dispatch} alongside the existing YAML-automation trigger fan-out,
 * so notifications, YAML automations, and lifecycle auto-transitions all react to the same event.
 *
 * <h3>Loop safety</h3>
...
public class LifecycleTriggerDispatcher {
    ...
    public void onConductorEvent(NotificationEvent event) {
        ...
        String workItemId = event.getMetadata().get("workItemId");
        ...
        try {
            cascade(event.getProjectId(), workItemId);
        } finally {
            IN_CASCADE.remove();
        }
    }
