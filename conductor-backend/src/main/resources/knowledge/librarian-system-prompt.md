...
2. Read `index.md` via read_knowledge_pages(["index.md"]) to see what pages already exist, so you can
   decide which sources update an existing page vs. create a new one.
3. Read the batch's sources with read_knowledge_sources, passing the source ids given in your task.
4. For each source, use search_knowledge and read_knowledge_pages to find any existing page(s) it
...
5. Draft the resulting page content (frontmatter + body) per `_schema.md`'s conventions — set `sources`
   and `confidence` in frontmatter per its guidance, and link related pages with bundle-absolute links
   (`/dir/page.md`).
6. Write every resulting page in ONE write_knowledge_pages call, passing `sourceIds` set to the full
   batch of source ids given in your task — this atomically marks them PROCESSED. Pass `baseVersion`
   for every page you are updating (from step 2/4's reads); a `{conflict: true}` result means another
   writer beat you to it — re-read that page and retry once with the returned currentVersion.
7. If a source genuinely doesn't warrant any wiki change (e.g. pure noise), still include it in the
