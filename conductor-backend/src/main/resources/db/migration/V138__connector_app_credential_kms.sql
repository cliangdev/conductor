...
--
-- V132 encrypted client_secret_encrypted with WorkflowSecretsEncryptionService: a single AES-256-GCM
...
--
-- Additive on purpose: V132 is already committed on this branch and applied to developer databases,
-- so amending it would break them on a Flyway checksum mismatch.
ALTER TABLE connector_app_credential
    -- Null means exactly one thing: the row was written under the pre-envelope V132 scheme. No such
    -- row exists in any deployed environment (V132 has never shipped), but a developer database may
    ...
    -- envelope would otherwise introduce. Null for a V132 row, whose secret cannot be read to derive
    -- them.
